...
Vector2 Vector2::operator*(const Matrix3x3& mat) const {
    float w = x * mat.m[2][0] + y * mat.m[2][1] + mat.m[2][2];
    assert(std::abs(w) > 1e-5f);
    return Vector2(
        (x * mat.m[0][0] + y * mat.m[0][1] + mat.m[0][2]) / w,
        (x * mat.m[1][0] + y * mat.m[1][1] + mat.m[1][2]) / w
    );
}

Vector2& Vector2::operator*=(const Matrix3x3& mat) {
    float tx = x * mat.m[0][0] + y * mat.m[0][1] + mat.m[0][2];
    float ty = x * mat.m[1][0] + y * mat.m[1][1] + mat.m[1][2];
    float w = x * mat.m[2][0] + y * mat.m[2][1] + mat.m[2][2];
    assert(std::abs(w) > 1e-5f);
    x = tx / w;
    y = ty / w;
...
Vector2 Matrix3x3::operator*(const Vector2& vec) const {
    float w = vec.x * m[2][0] + vec.y * m[2][1] + m[2][2];
    assert(std::abs(w) > 1e-5f);
    return Vector2(
        (vec.x * m[0][0] + vec.y * m[0][1] + m[0][2]) / w,
        (vec.x * m[1][0] + vec.y * m[1][1] + m[1][2]) / w
    );
}
